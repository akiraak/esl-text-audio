...
|---|---|---|---|---|
| 物語（Narrative） | 起承転結のある短い物語。登場人物・出来事の推移がある | A1〜C2（全レベル可） | 要 | 「読んでいて楽しい」を満たす主力形式。レベルが上がるほど心理描写・伏線・視点切り替えなどを加える |
| 対話文（Dialogue） | 2〜3人の会話形式。日常シーン（買い物・旅行・自己紹介など） | A1〜B1 | 要 | 短い発話の積み重ねで文長を自然に短く保てるため初級向き |
| 説明文（Descriptive） | 人・場所・物の様子を説明する | A1〜B2 | 要 | 形容詞・比較表現の練習に向く |
...
| 日記・手紙・メール（Personal writing） | 一人称視点の私的な文章 | A2〜B2 | 要 | 口語的表現・感情表現の練習に向く |
...
事実チェックは**全ジャンルで常に実施する**（新規トピックの `requiresFactCheck` は常に `true`）。
ただしチェックの対象はテキスト全体ではなく、**「現実世界について客観的事実として書かれている記述」のみ**とする。

（変更履歴: 当初は「物語・対話文＝フィクション」としてジャンル単位で一律対象外にしていたが、
架空の教室会話に実在の科学・歴史を載せるテキスト（例: トマト会話シリーズの1893年判決）が生まれ、
ジャンル単位の判定では実態に合わないため、2026-07-10 に内容単位のスコープ判定へ変更した）

- **チェック対象**: 実在の地名・人物・歴史的事実・日付・数値・科学的事実などの記述。
  **登場人物のセリフの中で語られる現実世界の事実も対象**（例: 会話文中で先生が説明する科学・歴史）
- **チェック対象外**:
  - 会話のやりとりそのもの（相づち・リアクション・冗談・言い回し）
  - 登場人物の意見・感想・好みなど主観的な内容
  - 架空の設定（架空の人物・場所・出来事。その設定内での描写を含む）
- **ユーモアの保全**: お笑い・ユーモアはこの教材の中核的な価値。明らかにジョークとして書かれた誇張・演出は
  事実の主張として扱わない。事実修正が必要な場合もジョーク・オチを消さない言い換えを最優先で探し、
  どうしても両立しない場合のみ正確性を優先する（その場合も別の形でユーモアを補えないか検討する）
- 本文（またはアウトラインの予定内容）に客観的事実の記述が一つも無い場合、`workflows/research.md` は「収集対象なし」として
  `sources/` 無しで先へ進めてよく、`workflows/factcheck.md` は本文にチェック対象が無いことを確認して「対象なし」で完了とする
...
## 理解度確認問題（Questions）の基準

記事の理解度確認問題（`variants/{level}-{tier}/questions/v{N}.json`）を作成するときの基準。
`workflows/questions.md`・`personas/question-writer.md` から参照する（設計の経緯は
[docs/plans/archive/article-questions.md](../plans/archive/article-questions.md) を参照）。

### 形式

- 4択の選択式（multiple choice）のみ。`choices` は4件固定、`answerIndex` は0始まり
  （True/False・穴埋めは将来の拡張。`type` フィールドで拡張余地だけ確保している）
- 問題文・選択肢・解説（explanation）はすべて英語で、**記事と同じCEFRレベルの語彙・文法
  （上表の該当レベル行）に収める**（問題文が本文より難しいと理解度確認にならない）
- 正解が本文の記述だけで一意に決まるようにする。本文にない外部知識を要求しない
  （`requiresFactCheck: true` の記事でも、問題の根拠は `sources/` ではなく本文とする）
- 誤答選択肢（distractor）は「本文を読んでいないと選びそうな、もっともらしいもの」にする。明らかなダミーは作らない
- 各問題に1〜2文の英語の解説を付け、正解の根拠が本文のどこにあるかを示す
- 正解位置（`answerIndex`）は問題間で偏らないようにする（同じ位置の連続や特定位置への集中を避ける）
...
- **内容理解（`type: "comprehension"`）**: 本文に書かれている事実・出来事・因果を問う。本文を読めば必ず答えられる
- **文脈語彙（`type: "vocabulary"`）**: 本文中の語句の意味を文脈から問う（"In the article, the word X means..."）
- **推論（`type: "inference"`）**: 本文から論理的に導ける内容を問う。**B2以上のバリアントのみ**使用する
  （B1以下の長文記事は内容理解・文脈語彙のみで8問を構成する）
