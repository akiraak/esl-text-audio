...
キャラを指定しない場合はスクリプトが自動で選ぶ（同トピック内の既存音声のキャラ使用回数を集計し、
最少のキャラを選択。同数の場合はランダム。トピック内で同じキャラばかりに偏らないようにするため）。
...
- 指定が無ければキャラ引数を省略し、スクリプトの自動選択（トピック内で使用回数が最少のキャラ）に任せる
- 利用者がトピック内で声を統一したいと言った場合のみ、既存音声の `audio/v{N}.json` の
  `character` を確認して同じキャラを明示指定する

### 3. 音声生成の実行

対象バリアント・確定済み記事バージョン N・キャラを指定して実行する（キャラ省略時はトピック内バランスで自動選択）。
