...
const TIER_LABELS = { normal: 'Normal', long: 'Long', 'very-long': 'Very Long' };
...
function renderLevels(basePath) {
  ...
  const body = `
  <nav class="back"><a href="${href(basePath, [])}">&larr; Back to list</a></nav>
...
function renderTextDetail(basePath, topicId) {
  ...
  const configRows = [
    ['Topic', config.topic],
    ['Topic Slug', config.topicSlug],
    ['Created At', config.createdAt],
    ['Genre', config.genre],
    ['Fact-Checked', config.requiresFactCheck ? 'Yes' : 'No'],
    ['Fact-Check Exemption Reason', config.factCheckExemptionReason || '-'],
  ]
  ...
  const matrixRows = LEVELS.map((level) => {
    const cells = TIERS.map((tier) => {
      ...
      return `<td class="unavailable">Not generated</td>`;
    }).join('');
...
function renderVariantDetail(basePath, topicId, variantId) {
  ...
  const variantRows = [
    ['Level', variant.level],
    ['Length', TIER_LABELS[variant.tier] || variant.tier],
    ['Target Word Count', variant.wordCountTarget],
    ['Long-form', variant.longForm ? 'Yes' : 'No'],
    ['Created At', variant.createdAt],
  ]
    .map(([k, v]) => `<tr><th align="left">${escapeHtml(k)}</th><td>${escapeHtml(v)}</td></tr>`)
    .join('\n');
  ...
  if (latestArticle !== undefined) {
    ...
    articlePreview = `
    <section class="block">
      <h2>Article (Latest: v${latestArticle})</h2>
  ...
  const body = `
  <nav class="back"><a href="${href(basePath, ['texts', topicId])}">&larr; Back to ${escapeHtml(config.topic)}</a></nav>
...
function renderArticle(basePath, topicId, variantId, version) {
  ...
  const body = `
  <nav class="back"><a href="${href(basePath, ['texts', topicId, variantId])}">&larr; Back to ${escapeHtml(config.topic)} (${escapeHtml(variant.level)}/${escapeHtml(TIER_LABELS[variant.tier] || variant.tier)})</a></nav>
