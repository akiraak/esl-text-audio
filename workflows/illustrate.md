...
`images/v{N}.png` を生成する。テキスト生成とは異なり、この段階のみ Claude Code が直接ペルソナとして本文を書くのではなく、
...
- `config.json` を読み、`topic` / `level` / `genre` / `requiresFactCheck` を把握する
- 確定済みの `articles/v{N}.md`（N は確定した本文のバージョン番号）を読み、記事全体を象徴する場面・雰囲気を把握する
...
- 保存先: `texts/{topic-slug}-{timestamp}/images/v{N}.prompt.txt`（N は手順2の本文バージョン番号。`images/` ディレクトリが
  無ければ作成する）
...
- 生成された `images/v{N}.png` を利用者に提示する（ビューアサーバ起動中であれば `/texts/{id}` で確認できる旨を案内する）
...
- `brushup.md` で本文が新バージョンとして更新された場合のイラスト再生成は現時点では自動化されていない。
  必要であれば、新しい本文バージョンに対してこのワークフローの手順2〜6を改めて実行する
