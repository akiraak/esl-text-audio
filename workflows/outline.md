...
- `texts/{topic-slug}-{timestamp}/config.json` が存在すること（トピック・ジャンルが確定済み）
- 本文で客観的事実を扱う予定のトピックの場合、`sources/` に [workflows/research.md](research.md) で取得した外部資料が保存済みであること
...
- `config.json` を読み、`topic` / `genre` を把握する
- `sources/` が存在する場合、そのファイル一覧にも目を通し、本文で使える事実要素を把握しておく
...
- 新規追加するセクションが客観的事実の記述を含む場合、`sources/` の範囲で裏付けられるか確認する。裏付けが取れない場合、そのセクションでは断定を避けるか、
...
### 8. セクション別ソース記録（客観的事実の記述を含むセクションのみ）

- 現実世界の客観的事実の記述（セリフの中で語られる事実を含む）を含むセクションごとに、根拠とする `sources/` のファイル名をアウトライン内に記録する
  （会話のやりとり・主観的内容・架空の設定だけのセクションには記録しない。スコープの定義は [esl-level-spec.md](../docs/specs/esl-level-spec.md) の「事実チェック方針」を参照）
...
- `根拠ソース` 行は客観的事実の記述を含むセクションのみに記載する。含まないセクションでは行自体を省略する
